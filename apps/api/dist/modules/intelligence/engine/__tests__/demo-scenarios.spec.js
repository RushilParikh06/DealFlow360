...
const TIER_PCT = { BRONZE: 5, SILVER: 10, GOLD: 15 };
const CATEGORY_PCT = { HARDWARE: 15, SERVICES: 10, SUBSCRIPTIONS: 8 };
/** Rebuilds what policy.seed.ts writes, min() included. */
function policiesFor(tier) {
    const rows = [
        {
    ...
    for (const category of Object.keys(CATEGORY_PCT)) {
        rows.push({
...
function evaluateSeeded(code, tier, rows) {
    const lines = buildLines(code, rows);
    return (0, evaluate_1.evaluateQuotation)({
        quotationId: code,
        currency: 'INR',
        tierId: tier,
        tierCode: tier,
...
//# sourceMappingURL=demo-scenarios.spec.js.map