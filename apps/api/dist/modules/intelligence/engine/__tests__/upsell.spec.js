...
describe('upsell ranking', () => {
    it('ranks on margin weighted by attach rate, not raw margin', () => {
        ...
        const ranked = (0, upsell_1.rankUpsell)([bigButRare, smallButCommon], 'INR');
    ...
    it('prices the suggestion at a ceiling-safe discount', () => {
        const ranked = (0, upsell_1.rankUpsell)([candidate({ productId: 'p', unitPriceMinor: 100_000, unitCostMinor: 50_000, safeDiscountBps: 1000, attachRateBps: 10_000 })], 'INR');
    ...
    it('drops anything already on the quote', () => {
        const ranked = (0, upsell_1.rankUpsell)([candidate({ productId: 'p1', alreadyOnQuote: true }), candidate({ productId: 'p2' })], 'INR');
    ...
    it('numbers the ranks and respects the limit', () => {
        ...
        const ranked = (0, upsell_1.rankUpsell)(many, 'INR', 3);
    ...
    it('is deterministic when two candidates tie completely', () => {
        ...
        expect((0, upsell_1.rankUpsell)([a, b], 'INR').map((r) => r.productId)).toEqual(['prd_a', 'prd_b']);
        expect((0, upsell_1.rankUpsell)([b, a], 'INR').map((r) => r.productId)).toEqual(['prd_a', 'prd_b']);
    });
    it('returns an empty list rather than throwing on no candidates', () => {
        expect((0, upsell_1.rankUpsell)([], 'INR')).toEqual([]);
    });
});
//# sourceMappingURL=upsell.spec.js.map