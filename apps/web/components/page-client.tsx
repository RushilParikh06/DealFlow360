...
function wireLogin(root: HTMLElement, page: PageName) {
  if (page !== "login") return;
  let role = "internal";
  root.querySelector("#role-internal")?.addEventListener("click", () => { role = "internal"; });
  root.querySelector("#role-customer")?.addEventListener("click", () => { role = "customer"; });
  root.querySelector("#submit-btn")?.addEventListener("click", () => {
    window.setTimeout(() => go(role === "customer" ? "portal" : "dashboard"), 1000);
  });
}
